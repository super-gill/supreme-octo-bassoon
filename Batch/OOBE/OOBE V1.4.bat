...
if %option%==no goto :vpn
if %option%==n goto :vpn
Echo.
...
echo BN Password reset to Ch1valry
echo.
set /P option=Do you want to add a VPN (yes/no)?
if %option%==yes goto :domaindeploy
if %option%==y goto :domaindeploy
...
if %option%==yes echo Domain deploy is no longer supported.
if %option%==y echo Domain deploy is no longer supported.
echo.
echo ========
echo complete
echo ========
...
for /L %%n in (20 -1 1) do (
  <nul set /P "=The PC will automatically reboot in %%n seconds!CR!"
...
for /f %%a in ('copy /Z "%~f0" nul') do set "CR=%%a"
for /L %%n in (20 -1 1) do (
  <nul set /P "=This message will self destruct in %%n seconds!CR!"
  ping -n 2 localhost > nul
)
exit